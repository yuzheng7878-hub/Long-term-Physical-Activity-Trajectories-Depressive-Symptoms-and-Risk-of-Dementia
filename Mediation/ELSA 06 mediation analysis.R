...
df <- df %>%
  mutate(
    has_prior_dementia = ifelse(dementia7 == 1, 1, 0)
  )

df_clean <- df %>% filter(has_prior_dementia == 0)

outcome_cols <- c("dementia8","dementia9","dementia10")
...
vars_to_impute <- c(
  ...
  "wealth_quartile", "drink_now", "smoke_2002", "n_chronic",
  "urbanicity", "r7cesd")
...
df_dementia$depression <- ifelse(df_dementia$r7cesd >= 3, 1, 0)
...
covariates <- c("age", "gender", "race", "uk_born")
...
simplified_results <- results_df %>%
  dplyr::select(Comparison, Mediator, N, Result_Type,
                ...
                ADE, ADE_P,  # <--- Added these two columns
